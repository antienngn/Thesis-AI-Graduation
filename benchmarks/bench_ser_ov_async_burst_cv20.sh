...
set -u
OV_CHUNK_SIZE_SMALL=2
OV_CHUNK_SIZE_LARGE=4
CV_VAL="${CV_VAL:-20}"
OUT_DIR="${OUT_DIR:-TEMP_RES_ASYNC_BURST_CV20_M128}"
...
for RATE in ${RATES}; do
    if [ "${RATE}" -le 8 ]; then
        OV_CHUNK_SIZE=${OV_CHUNK_SIZE_SMALL}
    else
        OV_CHUNK_SIZE=${OV_CHUNK_SIZE_LARGE}
    fi

    echo "Using OV_CHUNK_SIZE=${OV_CHUNK_SIZE} for RATE=${RATE}"
    run_one \
